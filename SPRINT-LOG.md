...
### 2026-07-11 UTC — run agent horaire (wave 40 adversarial : 400→410 cas)
- **Tenté** : item 1 — wave 40 : +10 cas adversariaux ciblant des angles inédits dans 10 domaines variés (fiscal LIFD 26 home office ATF 124 II 29, hybride CO 336 licenciement conjoint-employeur, social/assurances LAI 17 révision rente seuil 5pp, accident CO 58 glissade piscine pancarte insuffisante, assurances LCA 40 déchéance totale intention frauduleuse requise, circulation LCR 28 RC auto-école primaire sur élève, consommation CO 370 al.2 défauts cachés post-réception PV, sante LAMal 64a al.7 soins urgents maintenus malgré suspension, entreprise CO 530 société simple de fait food truck, voisinage CO 685 distances légales permis public ≠ droit privé)
- **Résultat** : passed ✓ — **410 cas dans `test/adversarial-cases.mjs`**, 3 gates verts, 0 doublon d'ID
- **Commits** : voir ci-dessous
- **Métriques** :
  - CI subset `LLM_MOCK=1` : **2645/2645 ✓** (aucun code modifié — données seulement)
  - Validation fiches : 0 erreur ✓ (314/314)
  - Benchmark JPT : **64.2/100 ✓** (gate >= 60)
  - Adversarial CLI sur 410 cas : non mesuré ce run (nécessite `claude -p` actif)
- **Nouveaux cas wave 40 (10)** :
  - `adv_fiscal_11` (LIFD 26 home office salarié : déduction refusée si bureau disponible à l'office — ATF 124 II 29 — ZH)
  - `adv_hybride_14` (CO 336 licenciement abusif par conjoint-employeur : double domaine bail+travail, contexte divorce — GE)
  - `adv_social_16` (LAI 17 révision rente AI : modification notable 5+ pp requise, légère amélioration ≠ suppression automatique — ATF 130 V 71 — BE)
  - `adv_accident_18` (CO 58 glissade piscine communale enfant 9 ans : pancarte insuffisante pour exonérer commune — ATF 130 III 213 — VD)
  - `adv_assurances_19` (LCA 40 déchéance totale cambriolage : intention de tromper doit être prouvée par assureur — ZH)
  - `adv_circulation_18` (LCR 28/58 leçon auto-école : RC du véhicule auto-école primaire, élève non détenteur — ZH)
  - `adv_consommation_20` (CO 370 al.2 salle de bain : réception sans réserve ne couvre pas les défauts cachés, 5 ans CO 371 — VD)
  - `adv_sante_26` (LAMal 64a al.7 suspension primes : soins urgents toujours couverts + LAMal 65 subsides cantonaux — GE)
  - `adv_entreprise_19` (CO 530/548 food truck : société simple de fait par actes concluants, propriété commune même titré au nom d'un — NE)
...
### 2026-07-13 UTC — run agent horaire (wave 42 adversarial : 420→430 cas + fix CI)
- **Tenté** : (1) fix CI : `test/ressources-reelles.test.mjs` n'avait pas de skip guard — le test réseau externe échouait en sandbox (403), bloquant la suite. Ajout de `skip: !process.env.TEST_SOURCES` conformément au commentaire du fichier ("ne tourne PAS dans la suite par défaut"). (2) wave 42 : +10 cas adversariaux ciblant des angles inédits dans 10 domaines (bail CO 266l formule officielle résiliation email nul, travail CO 336c suspension délai congé pendant maladie vs nullité, dettes LP 149 acte défaut biens ne libère pas la dette, famille CC 277 entretien post-majorité si formation en cours, étrangers LEI 47 délais regroupement 5 ans/12 mois dépassés, assurances LAMal 8 obligation acceptation refus illégal, consommation CO 40d délai révocation étendu si pas d'information, circulation LCR 33 priorité piéton uniquement sur passage balisé, santé LAMal 38 psychothérapie prescription médicale obligatoire, entreprise CO 800 responsabilité limitée SARL biens personnels protégés)
- **Résultat** : passed ✓ — **430 cas dans `test/adversarial-cases.mjs`**, 3 gates verts, 0 doublon d'ID
- **Note technique** : détachement HEAD ff16aad (origin/master) vs local master b9ea88b (wave 38). Résolution : git pull --ff pour rattraper wave 39-41 avant d'appliquer wave 42.
- **Métriques** :
  - CI subset `LEGAL_SAFE_MODE=0 LLM_MOCK=1` : **2728/2730 pass, 0 fail, 2 skip** ✓ (2 skips = test réseau + test SSL — comportement attendu en sandbox)
  - Validation fiches : 0 erreur ✓ (314/314)
  - Benchmark JPT : **66/100 ✓** (gate >= 60, ×6.6 vs LLM brut)
  - Adversarial CLI sur 430 cas : non mesuré ce run (nécessite `claude -p` actif)
- **Nouveaux cas wave 42 (10)** :
  - `adv_bail_43` (CO 266l résiliation bail d'habitation : email du proprio = nul, formule officielle cantonale obligatoire — GE)
  - `adv_travail_43` (CO 336c maladie pendant congé : résiliation nulle si donnée PENDANT arrêt, mais suspendue (non nulle) si maladie APRÈS résiliation — ZH)
  - `adv_dettes_38` (LP 149 acte de défaut de biens : titre de créance persistant 20 ans, dette non effacée, poursuite possible si biens retrouvés — VD)
  - `adv_famille_37` (CC 277 entretien post-majorité : obligation continue pendant formation même après 18 ans, salaire apprenti ne supprime pas la contribution — FR)
  - `adv_etrangers_32` (LEI 47 regroupement familial : délai 5 ans pour conjoint/enfants dépassé après 6 ans, délai 12 mois pour enfants >12 ans aussi dépassé — BS)
  - `adv_assurances_21` (LAMal 8 obligation d'acceptation : assureur de base ne peut pas refuser pour maladie préexistante — diabète type 1, refus = illégal — VD)
  - `adv_consommation_22` (CO 40d révocation vente à distance : sans information sur droit de retour → délai prolongé de 12 mois, pas seulement 14 jours — GE)
  - `adv_circulation_20` (LCR 33/49 piéton hors passage : priorité piéton uniquement sur passage balisé, hors passage = faute concomitante, responsabilité partagée — BE)
  - `adv_sante_28` (LAMal 38 psychothérapie : remboursée depuis juil. 2022 MAIS uniquement avec prescription médicale préalable, psychologue indépendant sans mandat = non remboursé — VD)
  - `adv_entreprise_21` (CO 800 SARL responsabilité limitée : associés ne répondent pas personnellement des dettes sociales sauf Nachschusspflicht statutaire ou garanties personnelles — GE)
- **Angles inédits wave 42** : CO 266l formule officielle cantonale obligatoire (mythe "email ou lettre suffisent pour résilier"), CO 336c suspension vs nullité (mythe "impossible d'être licencié quand on est malade"), LP 149 ADB ne libère pas (mythe "acte de défaut de biens = ardoise effacée"), CC 277 majorité ≠ fin obligation (mythe "18 ans = fin automatique pension alimentaire"), LEI 47 délais péremptoires (mythe "regroupement possible dès qu'on a les ressources"), LAMal 8 sans sélection risque (mythe "assureur peut refuser pour maladie préexistante"), CO 40d prolongation 12 mois sans info (mythe "14 jours sont absolus"), LCR 33 passage balisé uniquement (mythe "piétons ont la priorité partout"), LAMal 38 mandat médical obligatoire (mythe "toute psychothérapie remboursée depuis 2022"), CO 800 protection biens personnels SARL (mythe "associé SARL répond des dettes sur ses biens propres").
- **Prochaine action** : mesure éval CLI sur 430 cas au run suivant (nécessite `claude -p` actif). Validation juridique humaine (5 fiches gold + avocat) — hors scope autonomous.

### 2026-07-16 UTC — run agent horaire (wave 43 adversarial : 430→440 cas)
- **Tenté** : wave 43 : +10 cas adversariaux ciblant des angles inédits dans 10 domaines variés — focus sur les domaines sous-représentés (social : 3→5 cas, accident : 11→13 cas) et blind spots taxonomiques (aide sociale ≠ LACI/LAI, tort civil CO 55/58 ≠ LAA, société simple de fait CO 530)
- **Résultat** : passed ✓ — **440 cas dans `test/adversarial-cases.mjs`**, 3 gates verts, 0 doublon d'ID
- **Note technique** : premier edit effectué en détachement HEAD (ff16aad) → `git stash && git checkout master && git stash pop` a causé conflit merge (local master 23 commits derrière origin). Résolution propre : `git merge --abort`, `git restore` pour annuler, `git pull origin master` fast-forward, re-application manuelle des 10 cas sur master propre.
- **Métriques** :
  - CI subset `LEGAL_SAFE_MODE=0 LLM_MOCK=1` : **2728/2730 pass, 0 fail, 2 skip** ✓
  - Validation fiches : 0 erreur ✓ (314/314)
  - Benchmark JPT : **66/100 ✓** (gate >= 60, ×6.6 vs LLM brut)
  - Adversarial CLI sanity (10 cas, LLM_MOCK) : exit 0 ✓ (éval LLM réelle nécessite `claude -p`)
- **Nouveaux cas wave 43 (10)** :
  - `adv_social_17` (Cst 12 / LIAS 11 seuil fortune : chômage terminé 3 semaines, 5'800 CHF → démarche aide sociale cantonale, franchise fortune ~4'000 CHF — VD)
  - `adv_social_18` (Cst 12 / LIAS 26 héritage + aide : héritage 11'500 CHF en cours d'aide → obligation de déclarer, récupération sur succession si fortune dépasse seuil — GE)
  - `adv_accident_19` (CO 55 / CO 58 banc école cassé : bras cassé fils 11 ans sur banc défectueux — responsabilité État/commune CO 58, pas LAA — VD)
  - `adv_accident_20` (CO 55 / CO 41 glissade supermarché : cheville fracturée eau sur sol magasin — responsabilité du détenteur CO 55, pas assurance privée — BE)
  - `adv_violence_22` (CP 177 / CP 180 / CC 28b insultes racistes + menaces : voisin 8 mois d'injures et menaces → plainte pénale CP 177/180, mesures protection CC 28b — GE)
  - `adv_consommation_23` (CO 197 / CO 210 / CO 205 garantie légale 2 ans : robot 780 CHF panne 15 mois → garantie légale CO 197 (vice caché), délai avis CO 210, action CO 205 — ZH)
  - `adv_entreprise_22` (CO 530 / CO 62 / CO 41 sweat equity startup : 18 mois de travail sans contrat → société simple de fait CO 530 par actes concluants, restitution CO 62 ou indemnité CO 41 — GE)
  - `adv_circulation_21` (LCR 58 / LCR 65 voiture ami + RC exclusion : non-inscrit au contrat → RC tiers quand même valable (LCR 58 obligation RC), exclusion interne ≠ opposable à victime — ZH)
  - `adv_etrangers_33` (LEI 61a / LEI 43 Espagnol 9 ans permis B + chômage involontaire : ALCP Annexe I art. 6 maintien séjour 6 mois pour chercher emploi, permis B doit être renouvelé même si sans travail — GE)
...
### 2026-07-22 UTC — run agent horaire (wave 49 adversarial : 490→500 cas)
- **Tenté** : item 1 — wave 49 : +10 cas adversariaux ciblant des angles inédits dans 10 domaines — focus sur mythes procéduraux courants (défauts remise bail, clauses all-inclusive travail, mainlevée provisoire LP 82, MPUC urgence famille, prestataire UE <90j, modèle assurance maladie alternatif, distances plantations VD, subrogation SUVA vs recours civil, menaces électroniques CP 180, obligation continue LCR)
- **Résultat** : passed ✓ — **500 cas dans `test/adversarial-cases.mjs`**, 3 gates verts, 0 doublon d'ID
- **Note technique** : gate 1 nécessite `LEGAL_SAFE_MODE=0 LLM_MOCK=1` (le coupe-circuit `safe-mode.mjs` ajouté 2026-07-11 bloque les routes triage par défaut — tests d'intégration échouent sans ce flag). La commande dans le prompt schedulé est obsolète (ne spécifie pas `LEGAL_SAFE_MODE=0`) mais tous les runs récents utilisaient déjà ce flag.
- **Métriques** :
  - CI subset `LEGAL_SAFE_MODE=0 LLM_MOCK=1` : **2728/2728 pass, 0 fail, 2 skip** ✓
  - Validation fiches : 0 erreur ✓ (314/314)
  - Benchmark JPT : **66/100 ✓** (gate >= 60, ×6.6 vs LLM brut)
  - Adversarial CLI sur 500 cas : non mesuré ce run (nécessite `claude -p` actif)
- **Nouveaux cas wave 49 (10)** :
  - `adv_bail_48` (CO 258/259a défauts remise clés : refus prise de possession possible si graves, ne pas signer sans réserve — mythe "clés acceptées = défauts acceptés sans recours immédiat" — VD)
  - `adv_travail_48` (CO 321c/362 clause all-inclusive illimitée nulle : ATF 141 III 407, CO 362 impérativité défavorable travailleur — mythe "clause signée = jamais d'heures sup réclamables" — ZH)
  - `adv_dettes_44` (LP 80/82 mainlevée provisoire : reconnaissance de dette sous seing privé → mainlevée provisoire, définitive si débiteur inactif 20j — mythe "jugement au fond toujours requis" — VD)
  - `adv_famille_43` (CC 176/177 MPUC en quelques semaines : contribution d'entretien urgente sans attendre divorce, CPC 265 super-provisionnelles 24h si danger immédiat — mythe "pension alimentaire = attendre jugement divorce" — FR)
  - `adv_etrangers_37` (ALCP Annexe I art. 5 / LEI 11 prestataire UE <90j/an : annonce SWIPO, pas autorisation formelle — mythe "tout travail en Suisse = permis obligatoire même pour UE de courte durée" — GE)
  - `adv_sante_31` (LAMal 41 al. 4 / OAMal 29 modèle médecin de famille : restriction libre choix légale et librement consentie, exception urgence — mythe "assureur ne peut jamais restreindre le libre choix du médecin" — VD)
  - `adv_voisinage_30` (CC 688 / CcVD art. 91 distances plantations VD : thuyas 3m → distance min 1.5m (= 1/2 hauteur), droit cantonal prime — mythe "haies et conifères non couverts par règles de distance" — VD)
  - `adv_accident_23` (LAA 72 / CO 46/47 subrogation SUVA + recours civil complémentaire : SUVA récupère SES prestations, victime garde tort moral + dommage résiduel non couvert — mythe "SUVA paie = tous mes droits contre le tiers absorbés" — BE)
  - `adv_violence_25` (CP 180/181 menaces Instagram/réseaux sociaux : constitutives d'infraction pénale, jurisprudence TF établie — mythe "messages privés en ligne = pas de vraies menaces légales" — GE)
